  ...
  return (
    <div className="dashboard">
      ...
      <div className="page-header">
        <h2 className="page-heading">Finance</h2>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <Tab label={`Pending Payments (${payments.length})`} active={tab === "payments"} onClick={() => setTab("payments")} />
        <Tab label={`Pending Invoices (${invoices.length})`} active={tab === "invoices"} onClick={() => setTab("invoices")} />
      </div>
              ...
              <tbody>
                {payments.map((p, i) => (
                  <tr key={p.payment_request_id}>
                    ...
                    <td style={{ fontWeight: 600 }}>PKR {Number(p.amount).toLocaleString()}</td>
                    <td>{p.bank_name ?? "—"}</td>
                    <td style={{ fontFamily: "monospace", fontSize: 11 }}>{p.IBAN ?? "—"}</td>
              ...
              <tbody>
                {invoices.map((inv, i) => (
                  <tr key={inv.id}>
                    ...
                    <td style={{ fontWeight: 600 }}>PKR {Number(inv.amount).toLocaleString()}</td>
