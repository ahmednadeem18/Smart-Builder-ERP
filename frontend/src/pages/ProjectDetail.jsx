...
export default function ProjectDetail() {
        ...
        const handleDownloadPDF = () => {
                ...
                row("Subcontractor Budget", `PKR ${Number(p.subcontractor_cost).toLocaleString()}`);
                row("Actual Expenses", `PKR ${actual.toLocaleString()}`);
                row(isProfit ? "Remaining Budget" : "Over Budget",
                ...
                if (report.expenses.length > 0) {
                        ...
                        report.expenses.forEach((e) => {
                                tableRow([e.category, `PKR ${Number(e.total).toLocaleString()}`], [80, 80]);
                        });
                ...
                if (report.workers.length > 0) {
                        sectionTitle(`Workers (${report.workers.length})`);
                        tableHeader(["Name", "Type", "Rate", "Start", "End", "Status"], [38, 32, 25, 22, 22, 21]);
                        report.workers.forEach((w) => {
                                tableRow([
                                        w.name, w.type,
                                        `PKR ${Number(w.rate).toLocaleString()}`,
                                        w.start_date?.slice(0, 10),
                                        w.end_date?.slice(0, 10) ?? "Active",
                                        w.status
                                ], [38, 32, 25, 22, 22, 21]);
                        });
                }
                ...
                if (report.equipment.length > 0) {
                        ...
                        tableHeader(["Name", "Category", "Ownership", "Start", "End"], [40, 35, 30, 25, 25]);
                        report.equipment.forEach((e) => {
                                tableRow([
                                        e.name, e.category, e.ownership_type,
                ...
                if (report.materials.length > 0) {
                        ...
                        tableHeader(["Material", "Quantity", "Unit", "Supplier"], [45, 30, 25, 55]);
                        report.materials.forEach((m) => {
                                tableRow([m.material, m.quantity, m.unit, m.supplier], [45, 30, 25, 55]);
                        });
                }
                ...
                if (report.subcontractors.length > 0) {
                        ...
                        tableHeader(["Name", "Category", "Contract Price", "Payment"], [45, 40, 40, 30]);
                ...
                <div className="dashboard" id="report-content">
                        ...
                        <Section title="Project Information">
                                <div className="info-grid">
                                        ...
                                        <InfoRow label="End Date" value={p.end_date?.slice(0, 10) ?? "Ongoing"} />
                        ...
                        <Section title="Budget Overview">
                                <div className="stat-grid" style={{ padding: "16px" }}>
                                        ...
                                        <div className="stat-card"
                                                style={{ borderTop: `3px solid ${isProfit ? "#16a34a" : "#dc2626"}` }}>
                                                <p className="stat-label">{isProfit ? "Remaining" : "Over Budget"}</p>
                                                <p className="stat-value" style={{ color: isProfit ? "#16a34a" : "#dc2626" }}>
                                                        PKR {Math.abs(variance).toLocaleString()}
                                                </p>
                                        </div>
                                </div>
                                                ...
                                                <tbody>
                                                        {report.expenses.map((e, i) => (
                                                                <tr key={i}>
                                                                        ...
                                                                        <td style={{ fontWeight: 600 }}>PKR {Number(e.total).toLocaleString()}</td>
                                                                </tr>
                                                        ))}
                        ...
                        {/* Workers */}
                        <Section title={`Workers (${report.workers.length})`}>
                                {report.workers.length === 0 ? (
                                        <p className="empty">No workers allocated.</p>
                                ) : (
                                        <table className="table">
                                                <thead>
                                                        <tr>
                                                ...
                                                <tbody>
                                                        {report.workers.map((w, i) => (
                                                                <tr key={i}>
                                                                        ...
                                                                        <td>{w.type}</td>
                                                                        <td>{w.gender}</td>
                                                                        <td>PKR {Number(w.rate).toLocaleString()}</td>
